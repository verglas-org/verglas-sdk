...
```jsonc
{
  ...
  "pipelines": [
    { "binding": "EVENTS", "stream": "accepted_events" }
  ]
}
```
...
<Note>
A Verglas Stream binding inside a Durable Object uses the host's transactional
`storage.stream-send` path, not an external HTTP request. The accepted-row
mutation and publication are released through the same object event boundary;
a failed publication does not silently mark the ID delivered. Preserve the
event ID downstream as an additional idempotency key.
</Note>
