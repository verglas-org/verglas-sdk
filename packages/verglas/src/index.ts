...
import { printVerglasBanner } from "./wrangler-banner";
...
export function createCLIParser(argv: string[]) {
	...
	const showHelpWithCategories = async (): Promise<void> => {
		...
		for (const line of commandLines) {
			...
			const match = line.match(/^\s*verglas\s+(\S+)/);
			if (match) {
		...
		for (const category of registry.orderedCategories.keys()) {
			...
			const sortedCmdLines = Array.from(cmdLines).sort((a, b) => {
				const matchA = a.match(/^\s*verglas\s+(\S+)/);
				const matchB = b.match(/^\s*verglas\s+(\S+)/);
	...
	wrangler.command(
		...
		async (args) => {
			if (args._.length > 0) {
				...
				// option is present. This is to allow for running asynchronous printVerglasBanner
				// in the version command.
				if (args.v) {
					if (process.stdout.isTTY) {
						await printVerglasBanner();
					} else {
...
export async function main(argv: string[]): Promise<void> {
	...
	try {
		...
		try {
			// In the bootstrapper script `bin/verglas.js`, we open an IPC channel,
