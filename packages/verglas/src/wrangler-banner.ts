...
// The VERGLAS_PRERELEASE_LABEL is provided at esbuild time as a `define` for beta releases.
// Otherwise it is left undefined, which signals that this isn't a prerelease
declare const VERGLAS_PRERELEASE_LABEL: string;

export async function printVerglasBanner(
	...
	let text =
		typeof VERGLAS_PRERELEASE_LABEL === "undefined"
			? ` ⛅️ verglas ${verglasVersion}`
			: ` ⛅️ verglas ${verglasVersion} (${chalk.blue(VERGLAS_PRERELEASE_LABEL)})`;
	...
	if (semiver(process.versions.node, MIN_NODE_VERSION) < 0) {
		logger.warn(
			`Verglas requires at least Node.js v${MIN_NODE_VERSION}. You are using v${process.versions.node}. Please update your version of Node.js.`
		);
	}

	// Log a slightly more noticeable message if this is a major bump
	if (updateResult?.status === "update-available") {
		const currentMajor = parseInt(verglasVersion.split(".")[0]);
		const newMajor = parseInt(updateResult.latest.split(".")[0]);
		if (newMajor > currentMajor) {
			logger.warn(
				`The version of Verglas you are using is now out-of-date.
Please update to the latest version to prevent critical errors.
Run \`npm install --save-dev verglas@${newMajor}\` to update to the latest version.
After installation, run Verglas with \`npx verglas\`.`
			);
		}
	}
}
