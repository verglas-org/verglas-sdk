...
export class PipelineBinding {
	...
	constructor(bindingName, streamName, transport, options = {}) {
		...
		if (transactional) {
			if (!transport || typeof transport.streamSend !== "function") {
				throw new TypeError(
					"Durable Object Stream binding requires the WIT storage.stream-send transport"
				);
			}
		} else if (!transport || typeof transport.doFetch !== "function") {
			throw new TypeError(
				"Worker Stream binding requires the WIT bindings.do-fetch transport"
			);
		}
	 ...
	 * @param {{eventIds?: string[]}} [options]
	 * @returns {Promise<void>}
	 */
	async send(records, options = {}) {
		if (!Array.isArray(records)) {
			throw new TypeError(
				"Stream.send requires an Array of JSON-serializable records"
			);
		}
		...
		try {
			...
			throw new TypeError(
				`Stream.send requires JSON-serializable records: ${error.message}`
			);
		}
		if (encoded.byteLength > STREAM_MAX_REQUEST_BYTES) {
			throw new RangeError(
				`Stream.send request exceeds the 5 MiB encoded request limit (${encoded.byteLength} bytes)`
			);
		}
		...
		if (
			...
			throw new TypeError(
				"Stream.send eventIds must be 1-512 character strings matching the record count"
			);
		}
		if (this.#transactional) {
			if (eventIds !== undefined) {
				throw new TypeError(
					"Stream.send eventIds are unavailable inside a Durable Object transaction"
				);
			}
			await this.#transport.streamSend(
				this.#bindingName,
				this.#streamName,
				serialized
			);
		...
		if (!Number.isInteger(status) || status < 200 || status >= 300) {
			throw new Error(
				`Stream.send did not receive a durable ACK: HTTP ${String(result?.status)}`
			);
		}
	}
}
 ...
 */
export function createStreamBinding(
	bindingName,
	streamName,
	transport,
	options
...
function assertJsonValue(value, ancestors) {
	...
	if (typeof value === "number") {
		...
		throw new TypeError(
			"Stream.send requires JSON-serializable records: numbers must be finite"
		);
	}
	if (typeof value !== "object") {
		throw new TypeError(
			`Stream.send requires JSON-serializable records: unsupported ${typeof value}`
		);
	}
	if (ancestors.has(value)) {
		throw new TypeError(
			"Stream.send requires JSON-serializable records: cyclic value"
		);
	}
	ancestors.add(value);
	if (Array.isArray(value)) {
		...
		if (prototype !== Object.prototype && prototype !== null) {
			throw new TypeError(
				"Stream.send requires JSON-serializable records: objects must be plain"
			);
		}
		for (const key of Object.keys(value))
			assertJsonValue(value[key], ancestors);
		for (const symbol of Object.getOwnPropertySymbols(value)) {
			if (Object.prototype.propertyIsEnumerable.call(value, symbol)) {
				throw new TypeError(
					"Stream.send requires JSON-serializable records: symbol keys are not supported"
				);
			}
		}
	}
