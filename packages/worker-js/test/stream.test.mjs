...
test("Stream.send sends the named object a canonical JSON POST and awaits a 2xx ACK", async () => {
	...
	assert.deepEqual(
		JSON.parse(new TextDecoder().decode(calls[0].request.body)),
		[{ event: "one" }, { event: "two" }]
	);
});
...
test("Durable Object Stream.send stages records through storage before event commit", async () => {
	const calls = [];
	const stream = createStreamBinding(
		"STREAM",
		"stream-id",
		{
	...
	assert.deepEqual(calls, [
		{ binding: "STREAM", object: "stream-id", records: '[{"event":"staged"}]' },
	]);
});

test("Stream.send forwards stable producer event identities", async () => {
	const calls = [];
	const stream = createStreamBinding("STREAM", "events", {
		async doFetch(binding, object, request) {
			calls.push({ binding, object, request });
			return { status: 202, headers: [], body: new Uint8Array() };
		},
	});
	await stream.send([{ value: 1 }, { value: 2 }], {
		eventIds: ["run:0", "run:1"],
	});
	assert.deepEqual(calls[0].request.headers, [
		["content-type", "application/json"],
	...
	await assert.rejects(
		() => stream.send([{ value: 1 }], { eventIds: ["one", "two"] }),
		/record count/
	);
});
...
test("Stream.send rejects non-arrays, non-JSON values, and payloads over 5 MiB", async () => {
	...
	await assert.rejects(
		stream.send([{ payload: "x".repeat(MAX_REQUEST_BYTES) }]),
		/5 MiB/i
	);
